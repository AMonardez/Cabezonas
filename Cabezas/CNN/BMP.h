...
struct BMP {
    ...
    void join_points(uint32_t x0, uint32_t y0, uint32_t x1, uint32_t y1, uint8_t B, uint8_t G, uint8_t R, uint8_t A) {
        if (x0 > (uint32_t)bmp_info_header.width || y0 > (uint32_t)bmp_info_header.height ||
            x1 > (uint32_t)bmp_info_header.width || y1 > (uint32_t)bmp_info_header.height) {
            throw std::runtime_error("The point is outside the image boundaries!");
        }
        ...
        float dx_abs = abs(dx);
        float dy_abs = abs(dy);
        float division = dx_abs / dy_abs;
        double angulo = 90 - (atan(division) * 180 / 3.1415);
        ...
        for (int v = 0; v < steps; v++)
        {
            ...
            y = y + Yincrement;
            uint32_t channels = bmp_info_header.bit_count / 8;
            data[channels * ((uint32_t)round(y) * bmp_info_header.width + (uint32_t)round(x)) + 0] = B;
            data[channels * ((uint32_t)round(y) * bmp_info_header.width + (uint32_t)round(x)) + 1] = G;
            data[channels * ((uint32_t)round(y) * bmp_info_header.width + (uint32_t)round(x)) + 2] = R;
            if (channels == 4) {
                data[channels * ((uint32_t)round(y) * bmp_info_header.width + (uint32_t)round(x)) + 3] = A;
            }
        }
    }
